   ...
   Future<Map<String, dynamic>> studList(String jobId) async {
     ...
     try {
      ...
      final response = await http.post(
        Uri.parse(url),
        body: {
          'jobId': jobId, // Sending the user ID as a parameter
        },
      );
     ...
     if (response.statusCode == 200) {
        ...
        // Extract the jobs list from the response
        // Adjust this based on the actual structure of the response.
        List<dynamic> studLists = decodedResponse['stuDet'] ?? [];

        return {'stuDet': studLists};
      } else {
        print("Server error: ${response.statusCode}");
        return {'stuDet': []}; // Return an empty list on server error
      }
    } catch (e) {
      print("Error: $e");
return {'stuDet': []};    }
